...
function readAuth(file = CODEX_AUTH) {
...
function getTokenPayloads(auth) {
...
function extractAccountFingerprint(auth) {
  const { accessPayload, idPayload } = getTokenPayloads(auth);
  const authInfo =
...
function findDuplicateAccounts(targetName, fingerprint, meta) {
  const duplicates = [];
  ...
  for (const [otherName, otherInfo] of Object.entries(meta.accounts || {})) {
    ...
    if (savedAuth) {
      otherFingerprint = {
        ...otherFingerprint,
        ...extractAccountFingerprint(savedAuth),
      };
    }
...
function formatRelative(ts, { sameDayLabel = null } = {}) {
  if (!ts) return "从未";
...
function formatResetCountdown(unixSeconds) {
...
function colorizeRemaining(text, remaining) {
...
function formatApiReset(unixSeconds, resetAfterSeconds) {
...
function getAccountReferenceTime(info) {
...
function syncCurrentAccount(meta) {
...
function refreshAccountMetadata(meta, name) {
...
function refreshAllAccountMetadata(meta) {
  let changed = false;
...
function renderAccountHeader(name, info, isCurrent) {
...
async function refreshUsageForAccount(meta, name, { strict = false } = {}) {
...
async function refreshAllUsage(meta, { strict = false } = {}) {
  let changed = false;
  for (const name of Object.keys(meta.accounts || {})) {
...
async function promptAccountSelection(message, choices) {
...
async function cmdAdd(rawName) {
  ensureDir();
  const name = normalizeName(rawName);
  const auth = readAuth();

  if (!name) {
    console.error(chalk.red("用法: coa add <名字>"));
    process.exit(1);
  }
  ...
  if (duplicateNames.length) {
    console.error(chalk.red(`当前登录账号与已保存账号重复: ${duplicateNames.join(", ")}`));
    console.error(chalk.gray("请先确认你已经切换到正确的 Codex 账号，再执行 coa add <名字>"));
    process.exit(1);
  }
...
async function cmdList(options = {}) {
  ...
  if (refreshedMeta || syncedCurrent || refreshedUsage) {
    saveMeta(meta);
  }
  ...
  for (const name of names) {
    ...
    console.log(
      `    切换: ${formatRelative(info.lastUsed)}   保存: ${formatRelative(info.savedAt, { sameDayLabel: "今天" })}`
    );
  ...
  if (options.select) {
    ...
    if (chosen && chosen !== meta.current) {
      await cmdUse(chosen);
    }
  }
}

async function cmdUse(rawName) {
  ensureDir();
  const name = normalizeName(rawName);
  const meta = loadMeta();

  if (!name) {
    console.error(chalk.red("用法: coa use <名字>"));
    process.exit(1);
  }
  ...
  if (!meta.accounts[name]) {
    ...
    if (list.length) console.log(chalk.gray(`  可用账号: ${list.join("  ")}`));
    process.exit(1);
  }

  const src = accountPath(name);
  if (!fs.existsSync(src)) {
    console.error(chalk.red(`账号文件丢失，请重新执行 codex login 后再 coa add ${name}`));
    process.exit(1);
  }
...
async function cmdRemove(rawName) {
  const name = normalizeName(rawName);
  const meta = loadMeta();

  if (!name) {
    console.error(chalk.red("用法: coa remove <名字>"));
    process.exit(1);
  }
...
async function cmdCurrent(options = {}) {
  const meta = loadMeta();

  const syncedCurrent = syncCurrentAccount(meta);
  const refreshedCurrentMeta = meta.current ? refreshAccountMetadata(meta, meta.current) : false;
  const refreshedUsage =
    options.refresh && meta.current ? await refreshUsageForAccount(meta, meta.current, { strict: true }) : false;
  if (syncedCurrent || refreshedCurrentMeta || refreshedUsage) {
    saveMeta(meta);
  }
...
async function main() {
  switch (sub) {
    ...
    case "list":
    case "ls":
      await cmdList({ refresh, select: selectAfterList });
      break;
    case "use":
      await cmdUse(args[0]);
      break;
